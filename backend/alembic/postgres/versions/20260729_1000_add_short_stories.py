...
Revision ID: add_short_stories_pg
Revises: add_register_ip_pg
...
revision: str = 'add_short_stories_pg'
down_revision: Union[str, None] = 'add_register_ip_pg'
...
def upgrade() -> None:
    ...
    if not _table_exists(bind, 'short_stories'):
        ...
        op.create_index('ix_short_stories_user_id', 'short_stories', ['user_id'], unique=False)
...
def downgrade() -> None:
    ...
    if _table_exists(bind, 'short_stories'):
        if _index_exists(bind, 'short_stories', 'ix_short_stories_user_id'):
            op.drop_index('ix_short_stories_user_id', table_name='short_stories')
