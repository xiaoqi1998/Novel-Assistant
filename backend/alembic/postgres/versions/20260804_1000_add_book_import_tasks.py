"""新增拆书任务持久化表与项目拆书报告字段（PostgreSQL）

Revision ID: add_book_import_tasks_pg
Revises: add_char_current_location_pg
...
revision: str = 'add_book_import_tasks_pg'
down_revision: Union[str, None] = 'add_char_current_location_pg'
...
def upgrade() -> None:
    ...
    if not _table_exists(bind, 'book_import_tasks'):
        op.create_table(
            ...
            sa.Column('create_new_project', sa.Boolean(), nullable=True, server_default=sa.text('true'), comment='是否新建项目'),
...
def downgrade() -> None:
    ...
    if _column_exists(bind, 'projects', 'analysis_report'):
        op.drop_column('projects', 'analysis_report')
