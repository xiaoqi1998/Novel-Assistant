"""添加用户个人 Skill 表（SQLite）

Revision ID: add_user_skills_sqlite
Revises: add_character_arcs_sqlite
...
revision: str = 'add_user_skills_sqlite'
down_revision: Union[str, None] = 'add_character_arcs_sqlite'
...
def upgrade() -> None:
    ...
    if not _table_exists(bind, 'user_skills'):
        ...
        with op.batch_alter_table('user_skills', schema=None) as batch_op:
            ...
            batch_op.create_index('idx_user_skill', ['user_id', 'skill_key'], unique=True)
    ...
    if _table_exists(bind, 'user_skills'):
        with op.batch_alter_table('user_skills', schema=None) as batch_op:
            if _index_exists(bind, 'user_skills', 'idx_user_skill'):
                batch_op.drop_index('idx_user_skill')
