...
def _skill_constraint_block() -> str:
    """生成注入到 system_prompt 的爆款方法论约束块（默认合并去AI味约束）"""
    from app.services.skill_loader import get_deslop_constraints_cached
    deslop = get_deslop_constraints_cached()
    return (
...
def _purity_block() -> str:
    ...
    确保 AI 只输出纯故事正文，不夹带方法论术语、解释、标记、装饰符号等杂讯。
    """
    return (
        "\n\n【输出纯净性（最高优先级，必须严格遵守）】\n"
        "以上方法论仅作为你创作时的内部思考原则，严禁将方法论内容写入输出正文。\n"
        "你的输出必须是纯净的故事正文，直接可发布，必须满足：\n"
        "1. 严禁在正文中出现任何装饰性符号：禁止【】『』「」等括号包裹的标注、禁止——————/——/---/***/~~~等分隔线或装饰线；\n"
        "2. 严禁出现任何方法论术语（如：表线/里线、反转等级、爆点、情绪收益、交汇节点、黄金结构、去AI味、读者预期管理等）；\n"
        "3. 严禁出现任何解释、说明、总结、创作心得、前后缀、标题、Markdown标记、JSON或代码块；\n"
        "4. 严禁以『本段/本章/这一部分/以上方法论』等元叙述开头；\n"
        "5. 正文第一句即故事内容本身，最后一句即故事结尾，不得追加任何收尾语。"
    )
