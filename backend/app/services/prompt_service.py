...
**9. AI味分析 (AI Style Detection) - 检测AI写作痕迹**
以专业编辑视角检测本章文字的"AI生成感"，判断内容是否像AI写的。AI味不是文笔差，而是"太工整、太圆滑、太正确"，缺乏人写的毛边感。

判断维度（对照原文逐条核查）：
- **句式工整度**：是否大量使用对称排比、三段式结构、首尾呼应；是否存在过于规整的"总-分-总"结构
- **空洞形容词/副词**：是否滥用"缓缓""微微""深深""不禁""轻轻"等修饰词；是否大量使用"仿佛""似乎""如同"类比喻
- **情绪直描**：是否直接告诉读者情绪（如"他非常紧张""心中充满愤怒"），而非用动作/环境展示（Show, Don't Tell）
- **对话书面化**：台词是否过于书面、没有口语感；是否通篇"XX说道""XX问道"等标准标签
- **过度总结**：段尾是否频繁出现总结性收束句（如"这一刻，他终于明白……"）
- **信息密度均匀**：是否每段长度相近、节奏过于均匀，缺乏长短句交错
- **套路化心理描写**：是否用"脑海中浮现出……""一股暖流涌上心头"等模板化表达
- **无瑕疵感**：是否完全没有口语碎句、思考重复、犹豫语气等人味特征

输出要求：
- ai_score：AI味程度 0.0-1.0（0=完全人写，1=典型AI文风），支持两位小数
- level：按分数分级：0-0.2=低，0.2-0.4=中低，0.4-0.6=中等，0.6-0.8=中高，0.8-1.0=高
- signs：识别出的具体AI痕迹，至少1条（若无则空数组[]）。每条必须包含：
  - type：痕迹类型（如"排比对称句式"/"空洞形容词堆积"/"情绪直描"）
  - quote：从原文逐字复制的8-30字片段，作为证据
  - explanation：为什么这是AI味的说明
- strengths：写得有人味、自然的亮点（如"对话口语化""细节具体真实"），无则空数组[]
- suggestions：降低AI味的具体修改方向（结合原文给出），无则空数组[]
- 判定必须基于原文证据，禁止凭空臆断
...
{{
  ...
  "ai_style_analysis": {{
    "ai_score": 0.65,
    "level": "中高",
    "overall": "本章存在明显AI写作痕迹：句式工整、情绪直描偏多、对话过于书面，人味不足",
    "signs": [
      {{
        "type": "排比对称句式",
        "quote": "从原文逐字复制的8-30字片段",
        "explanation": "连续三句结构相同、字数相近，构成工整排比，是典型AI文风"
      }},
      {{
        "type": "情绪直描",
        "quote": "从原文逐字复制的8-30字片段",
        "explanation": "直接告诉读者情绪，没有通过动作/环境展示"
      }}
    ],
    "strengths": ["对话口语化较自然", "细节描写具体"],
    "suggestions": [
      "将排比句改为长短交错的碎句，保留其中一句作为强调即可",
      "把'他心中充满恐惧'改为'他指尖发凉，攥紧的拳头迟迟没松开'"
    ]
  }},
  "suggestions": [
...
✅ strengths 和 suggestions 为空时返回 []，不要返回 null
...
❌ suggestions 返回带编号对象、content对象、explanation对象等任何非字符串元素
    ...
    # AI去味默认提示词（基于 story-deslop Skill 的6道门禁规则）
    AI_DENOISING = """<system>
你是网文润色专家，专门去除AI写作痕迹，让文字回归自然、有人味。
...
门禁A：禁用词替换
- "眼中闪过一丝..." → "他垂下眼" / "眯起眼"
- "深吸一口气" → "胸口起伏了一下" / 直接删掉（90%无意义）
- "嘴角勾起一抹..." → "笑了一下，没到眼底" / "乐了"
- "缓缓开口" → "说" / 用动作引出对话
- 不能简单换成另一个形容词，要用"展示"替代"告诉"
...
<output>
【输出要求】
直接输出去味后的小说正文，不带任何解释、说明或报告。
保留原文的剧情走向、人物关系、场景设置，只改写表达方式。
