...
class Chapter(Base):
    ...
    # 用户反馈字段（用于质量反馈闭环）
    user_rating = Column(Integer, nullable=True, comment="用户对本章的评分 1-5")
    user_feedback = Column(Text, nullable=True, comment="用户对本章的文字反馈")
    user_feedback_at = Column(DateTime, nullable=True, comment="用户反馈时间")
