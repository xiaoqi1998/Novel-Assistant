...
class ChapterGenerateRequest(BaseModel):
    ...
    skill_key: Optional[str] = Field(None, description="Skill 标识（兼容旧版），指定后以该 Skill 的工作流指导创作")
    skill_keys: Optional[List[str]] = Field(None, description="Skill 标识列表（兼容旧版）")
    auxiliary_skill_keys: Optional[List[str]] = Field(None, description="辅助 Skill 标识列表（如去AI味、文风学习），只注入创作约束")
